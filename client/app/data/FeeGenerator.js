...
var assistantAlgo = function(service_code, minutes) {
  ...
  if (minutes > border1) {
    units =
      units +
      Math.ceil(Math.min(minutes - border1, border2 - border1) / 15.0) * 2;
  }
  if (minutes > border2) {
...
FeeGenerator.prototype.calculateFee = function(detail, row0, code) {
  ...
  var minutes = 0;
  if (
    code === this.normalizeCode(row0.code) &&
    detail.time_in &&
    detail.time_out
  ) {
    minutes =
      FeeGenerator.inMinutes(detail.time_out) -
      FeeGenerator.inMinutes(detail.time_in);
  ...
  if (
    ['K121A', 'K706A', 'K124A', 'K707A', 'K703A', 'K702A', 'K704A'].indexOf(
      code
    ) !== -1
  )
    return k121algo(service_code, minutes);
  if (['H409A', 'H410A'].indexOf(code) !== -1)
    return h409algo(service_code, minutes);
  return simpleAlgo(service_code, minutes);
};
